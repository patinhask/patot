...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'FN:PATINHAS\n' //NAMA LU
            + 'ORG:Owner Bot;\n' // NAMA CONTACT OWNER
            + 'TEL;type=CELL;type=VOICE;waid=5511934660977:+55 11 93466-0977\n' // NOMER OWNER
            + 'END:VCARD'
prefix = '#'//PREFIX BOT
blocked = ['11934660977']//NOMER BLOCK
limitawal = '50'
cr = '*patinhas domina*'
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Thauzinhoo otário @${num.split('@')[0]}, foi tarde 😂😂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '5511934660977@s.whatsapp.net'//Nomer Owner
                        ...
                        const ownerName = 'Patinhas'//Nama Owner
              ...
              await client.sendMessage(from, options, text)
               break
				...
				case 'marvellogo':
					...
					if (args.length < 1) return reply(`Enviar pedidos ${prefix}marvellogo texto, exemplo ${prefix}marvellogo Patinhas Criador`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'neonime':
					...
					break  
					case 'bpink':
                  if (args.length < 1) return reply(`Entrada de texto\Exemplo : ${prefix}Patinhas Bot`)
			...
			mus = body.slice(6)
               ...
               if (mus1.error) return reply(mus1.error)
				...
				case 'logowolf2':
					...
					if (args.length < 1) return reply(`teksnya mana? contoh ${prefix}logowolf pato|bot`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
